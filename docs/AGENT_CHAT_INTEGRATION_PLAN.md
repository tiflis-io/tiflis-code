...
|----------|--------|--------|----------|
| **Output format** | `--output-format stream-json` | `--output-format stream-json` | `-f json` |
| **Print mode** | `--print -p "prompt"` | `--print "prompt"` | `"prompt"` |
| **Verbose** | `--verbose` | — | — |
| **Session resume** | `--resume <id>` | `--resume <id>` | `-s <id>` |
...
{"type":"result","subtype":"success","total_cost_usd":0.003,"is_error":false,"duration_ms":1234,"num_turns":6,"result":"response","session_id":"abc123"}
```
