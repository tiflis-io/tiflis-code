...
final class KeyboardLayoutManager {
    ...
    func getRows(for layout: KeyboardLayout, modifiers: ModifierState = ModifierState()) -> [[KeyConfiguration]] {
        switch layout {
        case .letters:
            return getLettersLayout(language: currentLanguage)
        case .numbers:
    ...
    // MARK: - Letters Layouts

    /// Буквенная раскладка в зависимости от языка
    private func getLettersLayout(language: KeyboardLanguage) -> [[KeyConfiguration]] {
        switch language {
        case .english:
            return getEnglishLayout()
        case .russian:
            return getRussianLayout()
        case .georgian:
            return getGeorgianLayout()
        }
    }

    // MARK: - English Layout (QWERTY)

    /// Английская QWERTY раскладка - точная копия нативной iOS
    private func getEnglishLayout() -> [[KeyConfiguration]] {
        return [
    ...
    // MARK: - Russian Layout (ЙЦУКЕН)

    /// Русская ЙЦУКЕН раскладка - стандартная для iOS
    private func getRussianLayout() -> [[KeyConfiguration]] {
        return [
            // Ряд 1: Й Ц У К Е Н Г Ш Щ З (10 кнопок)
            [
                .letter("й"), .letter("ц"), .letter("у"), .letter("к"), .letter("е"),
                .letter("н"), .letter("г"), .letter("ш"), .letter("щ"), .letter("з")
            ],

            // Ряд 2: Ф Ы В А П Р О Л Д (9 кнопок, центрированы с отступами)
            [
                .letter("ф"), .letter("ы"), .letter("в"), .letter("а"), .letter("п"),
                .letter("р"), .letter("о"), .letter("л"), .letter("д")
            ],

            // Ряд 3: Shift Я Ч С М И Т Ь Б Backspace (8 букв + 2 функциональные)
            [
                KeyConfiguration(type: .modifier(.shift), width: .shift),
                .letter("я"), .letter("ч"), .letter("с"), .letter("м"),
                .letter("и"), .letter("т"), .letter("ь"), .letter("б"),
                KeyConfiguration(type: .special(.backspace), width: .shift)
            ],
    ...
    // MARK: - Georgian Layout (ქართული)

    /// Грузинская раскладка - стандартная QWERTY-подобная
    private func getGeorgianLayout() -> [[KeyConfiguration]] {
        return [
            // Ряд 1: ქ წ ე რ ტ ყ უ ი ო პ (10 кнопок)
            [
                .letter("ქ"), .letter("წ"), .letter("ე"), .letter("რ"), .letter("ტ"),
                .letter("ყ"), .letter("უ"), .letter("ი"), .letter("ო"), .letter("პ")
            ],

            // Ряд 2: ა ს დ ფ გ ჰ ჯ კ ლ (9 кнопок, центрированы с отступами)
            [
                .letter("ა"), .letter("ს"), .letter("დ"), .letter("ფ"), .letter("გ"),
                .letter("ჰ"), .letter("ჯ"), .letter("კ"), .letter("ლ")
            ],

            // Ряд 3: Shift ზ ხ ც ვ ბ ნ მ Backspace (7 букв + 2 функциональные)
            [
                KeyConfiguration(type: .modifier(.shift), width: .shift),
                .letter("ზ"), .letter("ხ"), .letter("ც"), .letter("ვ"),
                .letter("ბ"), .letter("ნ"), .letter("მ"),
                KeyConfiguration(type: .special(.backspace), width: .shift)
            ],
