...
describe("CONNECTION_TIMING", () => {
  describe("fast disconnect detection", () => {
    ...
    it("should have PONG_TIMEOUT_MS of 15 seconds (3 missed pings)", () => {
      expect(CONNECTION_TIMING.PONG_TIMEOUT_MS).toBe(15_000);
    });

    it("should have TIMEOUT_CHECK_INTERVAL_MS of 5 seconds for faster cleanup", () => {
      expect(CONNECTION_TIMING.TIMEOUT_CHECK_INTERVAL_MS).toBe(5_000);
    });

    it("should detect disconnect within ~5-8 seconds", () => {
      // With PING_INTERVAL=5s and PONG_TIMEOUT=15s (3 missed pings),
      // disconnect is detected after ~5-10 seconds (1-2 missed pings + timeout check)
      const minDetectionTime = CONNECTION_TIMING.PING_INTERVAL_MS;
      const maxDetectionTime =
        CONNECTION_TIMING.PONG_TIMEOUT_MS +
        CONNECTION_TIMING.TIMEOUT_CHECK_INTERVAL_MS;
  ...
  describe("timing relationships", () => {
    ...
    it("should have timeout check interval equal to or less than ping interval", () => {
      // Check interval should be frequent enough to detect timeouts promptly
      expect(CONNECTION_TIMING.TIMEOUT_CHECK_INTERVAL_MS).toBeLessThanOrEqual(
        CONNECTION_TIMING.PING_INTERVAL_MS
      );
    });
  });
