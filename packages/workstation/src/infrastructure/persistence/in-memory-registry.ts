...
export class InMemoryClientRegistry implements ClientRegistry {
  ...
  constructor(logger?: Logger) {
    this.logger = logger?.child({ component: 'ClientRegistry' });
  }
  ...
  registerTunnel(deviceId: DeviceId): Client {
      ...
      this.logger?.debug(
        {
          ...
          existingSubscriptions: existing.getSubscriptions(),
          isAuthenticated: existing.isAuthenticated,
        },
        'registerTunnel - returning existing client'
      );
      return existing;
    }
    ...
    this.clients.set(deviceId.value, client);
    this.logger?.debug(
      { deviceId: deviceId.value, totalClients: this.clients.size },
      'registerTunnel - created new client'
    );
    return client;
  }
